...
const createTripInfoTemplate = (points, offersByType, destinations) => {
  ...
  const createTripInfoDatesTemplate = () => {
    ...
    return `<p class="trip-info__dates">${startDate.format(startDateFormat)}&nbsp;&mdash;&nbsp;${endDate.format(DATE_FORMAT)}</p>`;
  };
  ...
  const createTripTotalPrice = () => {
    ...
    points.forEach((point) => {
      ...
      let totalPriceOfSelectedOffers = 0;
      const selectedOffers = pointTypeOffer.offers.filter((offer) => point.offers.includes(offer.id));
      selectedOffers.forEach((offer) => {
        totalPriceOfSelectedOffers += +offer.price;
      });

      totalPrice += totalPriceOfSelectedOffers;
    });
